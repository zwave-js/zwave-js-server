...
Returns a list of all known device classes.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_all_device_classes";
}
```

##### Get generic device class

Returns definition of generic device class corresponding to its key.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_generic_device_class";
  generic: number;
}
```

##### Get specific device class

Returns definition of specific device class corresponding to generic and specific device class keys.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_specific_device_class";
  generic: number;
  specific: number;
}
```
...
##### Get all indicator properties

Returns a list of all known indicator properties.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_all_indicator_properties";
}
```

##### Get indicator property

Returns indicator property with given id.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_indicator_property";
  id: number;
}
```
...
Returns a list of all known meters.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_all_meters";
}
```

##### Get all meter scales

Returns a list of all known meter scales for a given meter type.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_all_meter_scales";
  meterType: number;
}
```
...
Returns the meter of a given type.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_meter;
  type: number;
}
```
...
Return the meter name of a given meter type.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_meter_name";
  meterType: number;
}
```
...
Return meter scale for given meter type.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_meter_scale";
  type: number;
  scale: number;
}
```
...
```ts
interface {
  command: "registries.get_all_notifications";
}
```

##### Get notification

Returns notification of a given type.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_notification";
  type: number;
}
```
...
Returns the name of a given event for a given notification type

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_notification_event_name";
  type: number;
  event: number;
}
```
...
Returns name of a given notification type.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_notification_name";
  type: number;
}
```
...
Returns notification value for given notification.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_notification_value";
  notification: Notification;
  value: number;
}
```

##### Get notification value name

Returns the value name for a given notification and event type.

[compatible with schema version: 39+]

```ts
interface IncomingCommandRegistriesGetNotificationValueName
  extends IncomingCommandRegistriesBase {
  command: "registries.getNotificationValueName;
  type: number;
  event: number;
}
```
...
##### Get all named scale groups

Returns a list of all named scale groups.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_all_named_scale_groups";
}
```

##### Get named scale

Returns named scale of a given scale group and key.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_named_scale";
  group: keyof NamedScales;
  key: (keyof NamedScales[keyof NamedScales]) & number;
}
```

##### Get named scale group

Returns named scale group.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_named_scale_group";
  group: keyof NamedScales;
}
```
...
```ts
interface {
  command: "registries.get_all_sensors";
}
```

##### Get all sensor scales

Returns a list of all known sensor scales for a given sensor type.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_all_sensor_scales";
  sensorType: number;
}
```
...
Returns sensor of a given type.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_sensor";
  type: number;
}
```
...
Returns name of given sensor type.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_sensor_name";
  sensorType: number;
}
```
...
Returns sensor scale for a given sensor type.

[compatible with schema version: 39+]

```ts
interface {
  command: "registries.get_sensor_scale";
  type: number;
  scale: number;
}
```
