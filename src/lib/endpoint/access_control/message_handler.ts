...
export class EndpointAccessControlMessageHandler {
  async handle(
    rawMessage: IncomingMessageEndpoint,
    endpoint: Endpoint,
  ): Promise<EndpointAccessControlResultTypes[EndpointAccessControlCommand]> {
    const message = rawMessage as IncomingMessageEndpointAccessControl;
    switch (message.command) {
      ...
      case EndpointAccessControlCommand.getUser: {
        const user = await ensureAccessControl(endpoint).getUser(
          message.userId,
        );
        return { user };
      }
      case EndpointAccessControlCommand.getUserCached: {
        const user = ensureAccessControl(endpoint).getUserCached(
          message.userId,
        );
        return { user };
      }
